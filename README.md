...
The patch is the only record of the lines it removed, and it does not necessarily
record their line endings: git and the GitHub web editor normalise those in both
directions, so a patch made from a CRLF file often stores LF, and a patch written
entirely in CRLF carries `\r` into files that never had any. Restored lines are
therefore given the line ending the file itself uses, taken from the hunk's context
lines — those came from the file, not from the patch — or from the rest of the file
when a hunk has no context at all. Applying is deliberately left alone: it puts the
...
`rebase` also re-applies the patch to what it is about to write and refuses when
that does not reproduce the current tree.
...
default `~/.bun/install/cache`), the message names the store and tells you how to
move it back inside the project:
...
The refusal is correct in both cases: we do not write outside the project.
