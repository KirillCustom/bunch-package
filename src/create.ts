...
const EXCLUDE_PATTERNS = [
  ...
  '.patch-package.json',
  // Тем же занят bun: применяя патч, `bun install` кладёт в пакет пустой
  // маркер `.bun-tag-<хеш>` — измерено на пустом кеше, с нашим же патчем,
  // который никаких маркеров не несёт. Из этого маркера и растёт жалоба bun
  // #19327: следующий `bun patch --commit` видит его как новый файл и
  // записывает в патч, а тот кладёт его при установке, и так по кругу.
  // Сейчас маркер пуст, и `diff` о нём молчит; станет непустым или научимся
  // переносить пустые файлы — уедет в наш патч. `import` такие секции уже
  // выбрасывает, здесь то же самое с другой стороны.
  '.bun-tag-*',
...
// Патчи, созданные до 1.17.0, названы по манифесту. Переезжают они здесь — в
